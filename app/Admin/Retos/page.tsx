...
import { useRetosEquipoData } from '../../hooks/utils/useRetosEquipoData';

export default function PagePanel() {
  const { data, loading } = useRetosEquipoData();
      ...
      <div className="flex gap-6 items-start">
                ...
                data.porEquipo.map((equipo) => (
                  <div key={equipo.nombre} className="border-b border-[#C4649F]/20 last:border-0">
                    <div className="flex min-w-[900px] px-4 h-14 items-center gap-4 hover:bg-pink-50 transition-colors">
                      ...
                      <div className="w-60 shrink-0">
                        <p className="text-black text-sm">"{equipo.opcion1}"</p>
                      </div>
                      <div className="w-60 shrink-0">
                        <p className="text-black text-sm">"{equipo.opcion2}"</p>
                      </div>
                    </div>
                  </div>
                ))
        ...
        <div className="w-2/5 space-y-3">
          ...
          {data.metricas.length === 0 ? (
            <div className="text-center text-[#C4649F] text-sm py-8">
              ...
              >
                ...
                <div className="flex items-center gap-2 shrink-0">
                  <div className="bg-[#C4649F] rounded-full w-10 h-10 flex items-center justify-center">
                    <p className="text-white font-bold text-sm">{reto.total}</p>
                  </div>
